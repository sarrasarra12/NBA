...
export default function ClaimPage() {
  ...
  const capturePhoto = () => {
    ...
    canvas.toBlob((blob) => {
      const capturedFile = new File([blob], "carte-embarquement.jpg", { type: "image/jpeg" });
  ...
  const checkVolCompleted = () => {
    if (formData.flight_number && formData.departure_date && formData.departure_airport && formData.arrival_airport) {
  ...
  const checkReclamationCompleted = () => {
    if (formData.passenger_name && formData.email && formData.telephone && formData.description && typeContact) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({
          passenger_name: "", email: "", telephone: "", description: "",
          flight_number: "", departure_airport: "", arrival_airport: "",
          departure_date: "", pir_ref: "", category: "", pnr_code: ""
        });
        setFile(null);
        setAttachments([]);
        setLiéAuVol(null);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="relative bg-[#0A1628] px-4 sm:px-6 md:px-10 py-10 md:py-14 overflow-hidden">
        <img src={heroImg} alt="Avion" className="absolute inset-0 w-full h-full object-cover opacity-20" />
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 sm:py-8 md:py-10 space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-2xl border-2 border-gray-200 shadow-sm p-4 sm:p-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            {[{ value: true, label: "Oui" }, { value: false, label: "Non" }].map((option) => (
              <button
                ...
                className={`flex items-center justify-center sm:justify-start gap-2 px-6 py-3 rounded-xl border-2 font-semibold text-sm transition-all
                  ${liéAuVol === option.value ? "border-blue-500 bg-blue-50 text-blue-600" : "border-gray-200 text-gray-500 hover:border-blue-300"}`}
              >
                <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${liéAuVol === option.value ? "border-blue-500" : "border-gray-300"}`}>
                  {liéAuVol === option.value && <div className="w-2 h-2 rounded-full bg-blue-500" />}
                </div>
                {option.label}
              </button>
            ))}
        ...
        <div className="bg-gradient-to-br from-blue-200 to-blue-200 rounded-2xl shadow-xl p-6 text-blue-700 hover:shadow-2xl transition-all">
          <div className="hidden sm:flex items-center justify-between gap-6">
            ...
            <Link to="/suivi" className="bg-white text-blue-700 px-6 py-3 rounded-xl font-bold hover:bg-blue-50 hover:scale-105 transition-all shadow-lg flex items-center gap-2 whitespace-nowrap">
              <span>Suivre ma réclamation</span><span className="text-xl">→</span>
            </Link>
          </div>
          <div className="sm:hidden text-center">
            ...
            <Link to="/suivi" className="bg-white text-blue-700 px-6 py-3 rounded-xl font-bold hover:bg-blue-50 transition-all shadow-lg inline-flex items-center gap-2">
              <span>Suivre ma réclamation</span><span className="text-xl">→</span>
            </Link>
          </div>
        </div>
            ...
            <div className="p-4 sm:p-6 md:p-8">
              ...
              {!file ? (
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <button type="button" onClick={openCamera} className="block border-2 border-dashed border-blue-200 rounded-2xl p-6 text-center bg-blue-50 cursor-pointer hover:border-blue-500 hover:bg-blue-100 transition-all">
                    <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center text-3xl mx-auto mb-3 shadow-md">📸</div>
                ...
                <div className="bg-white rounded-2xl max-w-2xl w-full overflow-hidden shadow-2xl">
                  ...
                  <div className="p-6">
                    ...
                    <div className="relative bg-black rounded-xl overflow-hidden mb-4">
                      <video ref={videoRef} autoPlay playsInline muted className="w-full h-auto" style={{ maxHeight: '400px', objectFit: 'contain' }} />
                      <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                        <div className="border-4 border-white border-dashed rounded-lg opacity-50" style={{ width: '80%', height: '60%' }} />
                      </div>
                    </div>
        ...
        {/* INFOS VOL */}
        {liéAuVol === true && (
          <div className={`bg-white rounded-2xl border-2 shadow-sm overflow-hidden transition-all ${completed.vol ? "border-green-400" : "border-gray-200"}`}>
            ...
            <div className="p-4 sm:p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
              {[
                ...
                { label: "Réf PIR", name: "pir_ref", placeholder: "Ex: ", required: false }
              ].map((field) => (
                <div key={field.name}>
        ...
        {/* RÉCLAMATION */}
        {liéAuVol !== null && (
          <div className={`bg-white rounded-2xl border-2 shadow-sm overflow-hidden transition-all ${completed.reclamation ? "border-green-400" : "border-gray-200"}`}>
            <div className="bg-blue-50 px-4 sm:px-6 md:px-8 py-4 md:py-5 border-b border-gray-200">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 justify-between">
                <div className="flex items-center gap-3 sm:gap-4">
                  <div className="w-10 h-10 sm:w-11 sm:h-11 bg-blue-600 rounded-xl flex items-center justify-center text-lg sm:text-xl">📝</div>
            ...
            <div className="p-4 sm:p-6 md:p-8 grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
              ...
              <div className="col-span-1 sm:col-span-2">
                ...
                <select
                  value={typeContact}
                  onChange={(e) => { setTypeContact(e.target.value); setTimeout(checkReclamationCompleted, 0); }}
                  className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-sm focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-50 transition-all"
                >
              ...
              <div className="col-span-1 sm:col-span-2">
                <label className="block text-xs font-bold text-[#0A1628] uppercase tracking-wide mb-3">
                  Pièces jointes <span className="text-gray-400 font-normal normal-case">(optionnel)</span>
                </label>
                <div className="border-2 border-dashed border-gray-200 rounded-xl p-4 sm:p-6 bg-gray-50">
                      ...
                      {attachments.map((f, i) => (
                        <div key={i} className="flex items-center justify-between bg-white border border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-2.5">
                          <div className="flex items-center gap-2 text-xs sm:text-sm text-[#0A1628] overflow-hidden">
                            ...
                            <span className="text-gray-400 text-xs hidden sm:inline">({(f.size / 1024).toFixed(0)} KB)</span>
                          </div>
                          <button onClick={() => removeAttachment(i)} className="text-red-400 hover:text-red-600 font-bold text-lg transition-colors ml-2">×</button>
                        </div>
                      ))}
                    </div>
                  )}
                  <label className="flex items-center justify-center gap-2 cursor-pointer">
                    ...
                    <input type="file" className="hidden" multiple accept=".jpg,.jpeg,.png,.pdf,.doc,.docx" onChange={handleAttachments} />
                  </label>
                  <p className="text-center text-xs text-gray-400 mt-2">JPG, PNG, PDF, DOC — Max 10MB par fichier</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {pnrError && (
          <div className="bg-red-50 border-2 border-red-300 rounded-xl px-4 py-4 flex items-start gap-3">
            ...
            <button onClick={() => setPnrError('')} className="ml-auto text-red-400 hover:text-red-600 font-bold text-lg">×</button>
          </div>
        )}
        ...
        {liéAuVol !== null && (
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:justify-between mb-4 sm:mb-5">
              <button type="button" className="border-2 border-gray-200 text-gray-500 px-6 sm:px-8 py-3 sm:py-3.5 rounded-xl font-bold text-sm hover:border-red-300 hover:text-red-500 transition-all order-2 sm:order-1">
                ✕ Annuler
              </button>
              <button type="button" onClick={handleSubmit} className="bg-[#0A1628] text-white px-6 sm:px-8 py-3 sm:py-3.5 rounded-xl font-bold text-sm hover:bg-blue-700 transition-all order-1 sm:order-2">
                Soumettre la réclamation →
              </button>
            </div>
            <div className="text-center border-t border-gray-100 pt-4">
              <p className="text-xs sm:text-sm text-gray-500">
                Après soumission, vous recevrez <span className="font-semibold text-[#0A1628]">un email</span> et vous pourrez suivre l'état de votre réclamation.
              </p>
            </div>
          </div>
        )}

...
export default function ClaimPage() {
  ...
  const [completed, setCompleted] = useState({ vol: false, reclamation: false })
  ...
  const checkVolCompleted = () => {
    if (formData.flight_number && formData.departure_time &&
  ...
  const checkReclamationCompleted = () => {
    if (formData.passenger_name && formData.email &&
        formData.telephone && formData.description && typeContact) {
      ...
      <nav className="bg-[#0A1628] px-4 sm:px-6 md:px-10 py-3 sm:py-4 flex justify-between items-center sticky top-0 z-50">
        ...
        <div className="flex gap-2 sm:gap-4">
          {['FR', 'العربية', 'EN'].map(lang => (
            <button key={lang} className="text-gray-400 hover:text-white text-xs sm:text-sm font-semibold transition-colors">
              {lang}
            </button>
          ))}
        </div>
      ...
      <div className="relative bg-[#0A1628] px-4 sm:px-6 md:px-10 py-10 md:py-14 overflow-hidden">
        ...
        <div className="relative z-10 max-w-lg">
          ...
          <p className="text-gray-400 text-sm">
            Notre IA analyse votre carte d'embarquement et pré-remplit le formulaire automatiquement.
          </p>
        </div>
      </div>
            ...
            <div className="p-4 sm:p-6 md:p-8">
              ...
              <label className="block border-2 border-dashed border-blue-200 rounded-2xl p-6 sm:p-8 md:p-12 text-center bg-blue-50 cursor-pointer hover:border-blue-500 hover:bg-blue-100 transition-all">
                ...
                <h3 className="text-sm sm:text-base font-bold text-[#0A1628] mb-2">
                  {file ? ` ${file.name}` : "Glissez votre carte d'embarquement ici"}
                </h3>
                <p className="text-xs sm:text-sm text-gray-500 mb-4">JPG, PNG, PDF — Max 10MB</p>
          ...
          <div className={`bg-white rounded-2xl border-2 shadow-sm overflow-hidden transition-all ${completed.vol ? 'border-green-400' : 'border-gray-200'}`}>
            ...
            <div className="p-4 sm:p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
              {[
                ...
                { label: 'Date du vol', name: 'departure_time', type: 'date', required: true },
                ...
                { label: 'Réf PIR', name: 'pir_ref', placeholder: 'Ex: TUNBJ12345', required: false },
              ].map(field => (
                <div key={field.name}>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 sm:p-6">
            ...
            <div className="text-center border-t border-gray-100 pt-4">
              <p className="text-xs sm:text-sm text-gray-500">
                 Après soumission, vous recevrez un <span className="font-semibold text-[#0A1628]">token unique</span> par email pour suivre l'état de votre réclamation en temps réel.
              </p>
            </div>
          </div>
        )}
